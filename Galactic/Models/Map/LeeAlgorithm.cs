    ...
    class LeeAlgorithm
    {
        ...
        private void FindStartCell(out int startX, out int startY)
        {
            ...
            for (int i = 0; i < w; i++)
            {
                for (int j = 0; j < h; j++)
                {
                    if (ArrayGraph[i, j] == (int)TypePoint.StartPosition)
                    {
                        startX = i;
                        startY = j;
                        return;
                ...
                for (int i = 0; i < w; i++)
                {
                    for (int j = 0; j < h; j++)
                    {
                        if (ArrayGraph[i, j] == _step)
                        {
                            // Пометить все соседние свободные непомеченные ячейки числом d + 1
                            if (i != w - 1)
                                if (ArrayGraph[i + 1, j] == (int)TypePoint.EmptySpace) ArrayGraph[i + 1, j] = _step + 1;
                            if (j != h - 1)
                                if (ArrayGraph[i, j + 1] == (int)TypePoint.EmptySpace) ArrayGraph[i, j + 1] = _step + 1;
                            if (i != 0)
                                if (ArrayGraph[i - 1, j] == (int)TypePoint.EmptySpace) ArrayGraph[i - 1, j] = _step + 1;
                            if (j != 0)
                                if (ArrayGraph[i, j - 1] == (int)TypePoint.EmptySpace) ArrayGraph[i, j - 1] = _step + 1;
                            ...
                            if (i < w - 1)
                                if (ArrayGraph[i + 1, j] == (int)TypePoint.Destination)
                                {
                                    _finishPointI = i + 1;
                                    _finishPointJ = j;
                                    finished = true;
                            ...
                            if (j < h - 1)
                                if (ArrayGraph[i, j + 1] == (int)TypePoint.Destination)
                                {
                                    _finishPointI = i;
                                    _finishPointJ = j + 1;
                                    finished = true;
